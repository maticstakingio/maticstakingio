    ...
    public class PropertyManager<T>
    {
        ...
        public virtual void WriteToXlsx(ExcelWorksheet worksheet, int row, int cellOffset = 0, ExcelWorksheet fWorksheet = null)
        {
            ...
            foreach (var prop in _properties.Values)
            {
                ...
                else if (prop.IsHyperlink)
                {
                    cell.Style.Font.Color.SetColor(Color.Blue);
                    cell.Formula = "HYPERLINK(\"" + prop.GetProperty(CurrentObject) + "\",\"" + prop.GetProperty(CurrentObject) + "\")";
                }
                else if (prop.IsHyperlinkAppend)
                {
                    if (prop.GetProperty(CurrentObject) == null || string.IsNullOrEmpty(prop.GetProperty(CurrentObject).ToString()))
                        continue;

                    var objArr = prop.GetProperty(CurrentObject).ToString().Split(',');

                    if (objArr.Length < 2)
                        continue;

                    cell.Style.Font.Color.SetColor(Color.Blue);
                    cell.Formula = "HYPERLINK(\"" + objArr[0] + "\",\"" + objArr[1] + "\")";
